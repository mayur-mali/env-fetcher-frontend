  ...
  const renderPaginationButtons = () => {
    ...
    if (totalPages > 5) {
      ...
      if (currentPage + 1 > 5) {
        buttons.unshift(
          <button
            className="px-3 py-1 cursor-pointer mx-1 text-[#1B163B] text-sm font-normal"
      ...
      if (currentPage > 3) {
        buttons.unshift(
          <button
            key="prev"
            className="px-3 cursor-pointer py-1 mx-1 text-[#1B163B] text-sm font-normal"
          ...
          .map((r?: any) =>
            r
              ? [].slice.call(r?.children).map((c: any, i: number) => {
                  if (
                    c.children[0]?.children[0]?.children[0]?.nodeName ===
                    "BUTTON"
                  ) {
                    return "-";
                  }
